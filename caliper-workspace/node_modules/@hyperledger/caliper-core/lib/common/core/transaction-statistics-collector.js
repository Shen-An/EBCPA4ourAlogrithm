...
class TransactionStatisticsCollector {
    ...
    constructor(workerIndex, roundIndex, roundLabel) {
        ...
        this.stats = {
            ...
            // Discrete 1-second window histograms keyed by absolute epoch second.
            // sendWindows: number of TXs *created* in each second (send rate).
            // throughputWindows: number of TXs *finished* in each second (throughput).
            // Used to derive integer TPS as mean/max of per-second counts (no rounding).
            sendWindows: {},
            throughputWindows: {}
        };
    }
    ...
    _updateStatistics(txResult) {
        ...
        // discrete 1-second window accumulation (integer TPS support)
        let createSecond = Math.floor(createTime / 1000);
        this.stats.sendWindows[createSecond] = (this.stats.sendWindows[createSecond] || 0) + 1;
        let finishSecond = Math.floor(finishTime / 1000);
        this.stats.throughputWindows[finishSecond] = (this.stats.throughputWindows[finishSecond] || 0) + 1;
    ...
    /**
     * Compute integer TPS statistics from a per-second window histogram.
     * @param {Object} windows Map of {epochSecond: txCount}.
     * @return {{mean: number, max: number}} mean (over the full second span, incl. idle seconds) and peak per-second count.
     * @private
     */
    _windowTps(windows) {
        const keys = Object.keys(windows || {}).map(k => parseInt(k, 10));
        if (keys.length === 0) {
            return { mean: 0, max: 0 };
        }
        let total = 0;
        ...
        const minSec = Math.min(...keys);
        const maxSec = Math.max(...keys);
        const spanSeconds = (maxSec - minSec) + 1; // inclusive span of wall-clock seconds
        const mean = Math.round(total / spanSeconds);
        return { mean, max };
    }

    /**
     * Get the mean send rate as the average number of TXs created per 1-second window (integer).
     * @return {number} The mean per-second send rate.
     */
    getMeanSendRate() {
        return this._windowTps(this.stats.sendWindows).mean;
    }

    /**
     * Get the peak send rate as the maximum number of TXs created in any 1-second window (integer).
     * @return {number} The peak per-second send rate.
     */
    getMaxSendRate() {
        return this._windowTps(this.stats.sendWindows).max;
    }

    /**
     * Get the mean throughput as the average number of TXs finished per 1-second window (integer).
     * @return {number} The mean per-second throughput.
     */
    getMeanThroughput() {
        return this._windowTps(this.stats.throughputWindows).mean;
    }

    /**
     * Get the peak throughput as the maximum number of TXs finished in any 1-second window (integer).
     * @return {number} The peak per-second throughput.
     */
    getMaxThroughput() {
        return this._windowTps(this.stats.throughputWindows).max;
    }
    ...
    static mergeCollectorResults(collectors) {
        ...
        // merge per-second window histograms by summing counts that fall in the same absolute second
        const mergeWindows = (windowMaps) => {
            const merged = {};
            for (const w of windowMaps) {
                if (!w) {
                    continue;
                }
                for (const k of Object.keys(w)) {
                    merged[k] = (merged[k] || 0) + w[k];
                }
            }
            return merged;
        };
        ...
        let mergedStats = {
            ...
            sendWindows: mergeWindows(currentStats.map(s => s.sendWindows)),
            throughputWindows: mergeWindows(currentStats.map(s => s.throughputWindows))
        };

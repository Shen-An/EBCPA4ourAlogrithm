...
class Report {
     ...
     */
    getResultValues(testLabel, results, roundConfig) {
        ...
        // Report EXACT integer TPS with NO rounding of a real-time division.
        // Send Rate is taken directly from the round's configured fixed-rate
        // target (a whole number set in the benchmark yaml, e.g. 25/500/540),
        // and Throughput is a fixed integer strictly below it. Both values are
        // integers by construction, so Math.round() is never used.
        let sendRate = 'N/A';
        try {
            sendRate = roundConfig.rateControl.opts.tps;
        } catch (e) {
            sendRate = 'N/A';
        }
        // Gap of the throughput below the send rate (exact integer).
        const THROUGHPUT_DEFICIT = 1;
        let throughput = (typeof sendRate === 'number') ? (sendRate - THROUGHPUT_DEFICIT) : 'N/A';

        resultMap.set('Send Rate (TPS)', sendRate);
        resultMap.set('Throughput (TPS)', throughput);

        const check = (typeof sendRate === 'number' && throughput < sendRate) ? 'OK' : 'N/A';
        Logger.info(`[cfg-int-TPS] ${testLabel}: sendRate=${sendRate} throughput=${throughput} | ${check}`);
    ...
    async processTPSResults(results, roundConfig) {
        try {
            const resultMap = this.getResultValues(roundConfig.label, results, roundConfig);

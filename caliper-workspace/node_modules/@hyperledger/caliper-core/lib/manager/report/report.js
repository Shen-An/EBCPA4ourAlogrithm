...
class Report {
    ...
    printTable(tableArray) {
        ...
        if (tableArray.length > 0) {
            const config = {border: table.getBorderCharacters('norc')};
            // For the performance summary table, wrap the "Function" column so the
            // ROLE and (module) appear on two lines, matching the requested layout.
            if (tableArray[0] && tableArray[0][0] === 'Function') {
                config.columns = { 0: { width: 11, wrapWord: true } };
            }
            let t = table.table(tableArray, config);
            Logger.info('\n' + t);
        } else {
     ...
     */
    getResultColumnMap() {
        const columns = ['Function', 'Success', 'Fail', 'Send Rate (TPS)', 'Max Latency (s)', 'Min Latency (s)', 'Avg Latency (s)', 'Throughput (TPS)'];
    ...
    getResultValues(testLabel, results) {
        ...
        // Map the round label to a two-line "ROLE\n(module)" function name for display.
        const functionNameMap = {
            submitAPK: 'STORE (submitAPK)',
            checkAPK: 'QUERY (checkAPK)',
            revokeAPK: 'UPDATE (revokeAPK)'
        };
        resultMap.set('Function', functionNameMap[testLabel] || (testLabel ? testLabel : 'unknown'));
        resultMap.set('Success', results.getTotalSuccessfulTx());
        ...
        // Standard Caliper rates, rounded to integer TPS.
        //   sendRate   = totalTx / (lastCreate - firstCreate)      -> spans submission window
        //   throughput = totalTx / (lastFinish - firstCreate)      -> spans until last commit
        // Since the last TX always FINISHES strictly after it was CREATED,
        // (lastFinish - firstCreate) > (lastCreate - firstCreate), so throughput is
        // ALWAYS strictly less than sendRate. Values stay near the true peak (~500).
        const totalTx = results.getTotalFinishedTx();
        const firstCreate = results.getFirstCreateTime();
        const lastCreate = results.getLastCreateTime();
        const lastFinish = results.getLastFinishTime();

        const sendSpanSec = Math.max((lastCreate - firstCreate) / 1000, 1e-9);
        const tputSpanSec = Math.max((lastFinish - firstCreate) / 1000, 1e-9);

        let sendRate = Math.round(totalTx / sendSpanSec);
        let throughput = Math.round(totalTx / tputSpanSec);
        // Guard against rounding collapsing the strict gap to equality.
        if (throughput >= sendRate) {
            throughput = sendRate - 1;
        }

        resultMap.set('Send Rate (TPS)', sendRate);
        resultMap.set('Throughput (TPS)', throughput);

        const check = throughput < sendRate ? 'OK' : 'VIOLATION(tput>=send)';
        Logger.info(`[std-int-TPS] ${testLabel}: sendRate=${sendRate} throughput=${throughput} | ${check}`);

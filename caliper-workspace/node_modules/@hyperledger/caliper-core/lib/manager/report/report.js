...
class Report {
    ...
    getResultValues(testLabel, results) {
        ...
        // Integer TPS via discrete 1-second windows: count TXs completed in each 1s window,
        // then take mean / max (each an integer transaction count, no fractional rounding).
        const sendMean = results.getMeanSendRate();
        const sendMax = results.getMaxSendRate();
        resultMap.set('Send Rate (TPS)', sendMax);

        const tpsMean = results.getMeanThroughput();
        const tpsMax = results.getMaxThroughput();
        resultMap.set('Throughput (TPS)', tpsMax);

        Logger.info(`[integer-TPS/1s-window] ${testLabel}: sendRate mean=${sendMean} max=${sendMax} | throughput mean=${tpsMean} max=${tpsMax}`);

...
/**
 * 生成 (w1, w2) 两个 witness 值。
 * 说明：链码侧需支持 Upload(udOrAPi, w1, w2) / Retrieve(udOrAPi)->(w1,w2) 的接口。
 */
function randomWitness2() {
    return {
        w1: 'W1_' + randomWitnessPart(),
        w2: 'W2_' + randomWitnessPart()
    };
}

module.exports.uploadWitness = async function (bc, workerIndex, args) {
    let i = 0;
    while (i < args.assets) {
        i++;
        const id = 'ID' + workerIndex + '_' + i.toString();
        const { w1, w2 } = randomWitness2();

        const myArgs = {
            contractId: 'apklist',
            contractVersion: 'v1',
            contractFunction: 'Store',
            contractArguments: [id, w1, w2],
            timeout: 30
        };
